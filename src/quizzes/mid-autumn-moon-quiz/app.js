(() => {
  ...
  const LEGENDARY = { name: '月兔天選・萬福十全糬', blessing: '萬福集結 · 十全中秋', meaning: '月兔灑下萬福金粉，十種月宮好運同時落進你的石臼。願財運、心動、貴人、團圓與心安，全都在這個中秋向你靠近。' };
  ...
  function drawResultPastry(picks) {
    ...
    pastryImage.onload = () => {
      ...
      ctx.globalCompositeOperation = 'color'; ctx.fillStyle = mixedColor(picks); ctx.fillRect(70, 145, 295, 195);
      ctx.restore();
  ...
  function showResult() {
    ...
    $('#result-ingredients').innerHTML = state.picks.map((pick, index) => `<span>${index === 0 ? '主' : '副'}·${pick.icon} ${pick.name}</span>`).join('');
    $('#result-supports').innerHTML = `<span class="support-label">副效果</span>${state.picks.slice(1).map((pick) => `<p class="result-support"><b>${pick.name}：</b>${SUPPORT_BLESSINGS[pick.result].suffix}</p>`).join('')}`;
    drawResultPastry(state.picks);
    showScreen('result');
  }
